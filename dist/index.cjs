...
var handleManyFar = async (proxies) => {
  ...
  const replaceProxies = async (added, allProxies) => {
    ...
    const newFunctions = await import_swiss_ak.PromiseUtils.map(added, (p) => getFunctionList(p));
  ...
  const callFunction = (fnName) => async (...fnArgs) => {
    return import_swiss_ak.PromiseUtils.map(proxyList, (proxy) => callFunctionOnProxy(proxy, fnName, fnArgs));
  };
  const callObj = Object.fromEntries(functions.map((fnName) => [fnName, callFunction(fnName)]));
  return {
    callObj,
...
var handleExit = (proxy) => {
  const deferred = (0, import_swiss_ak2.getDeferred)();
